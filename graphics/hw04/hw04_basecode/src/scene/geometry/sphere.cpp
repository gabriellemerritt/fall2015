...
void Sphere::ComputeArea()
{
    ...
    //approx for an ellipsoid
    float a = 0.5f*transform.getScale().x;
    float b = .5f*transform.getScale().y;
    float c = .5f*transform.getScale().z;
    float S = 4*PI*powf(((powf(a*b,1.6f ) + powf(a*c,1.6f ) + powf(c*b,1.6f ))/3.0f), 1.0/1.6f);
    area = S;
}
Intersection Sphere::RandomSample(float randX, float randY)
{
    Intersection isx;
    float theta = 2.0*PI*randX;
    float u = randY - 0.5f;
    float r = glm::sqrt(1 - powf(u, 2.0f));
    glm::vec3 rand_point = glm::vec3(r*glm::cos(theta), r*glm::sin(theta), u );
...
Intersection Sphere::GetIntersection(Ray r)
{
    ...
    if(t >= 0)
    {
        ...
        float r = glm::length2(glm::vec3(P));
        float theta = glm::acos( P[2]/r);
        float phi = glm::atan(P[1],P[0]) + PI/2.0f;
        glm::vec3 obj_tan(glm::sin(theta)*glm::cos(phi), glm::sin(theta)*glm::sin(phi), glm::cos(theta));
