...
void Triangle::ComputeArea()
{
   ...
   area = glm::length(glm::cross((points[1]- points[0]), (points[2]-points[0])))/2.0f;
}

void Mesh::ComputeArea()
{
    //Extra credit to implement this
    //sum up areas of triangles in mesh
...
Intersection Triangle::GetIntersection(Ray r){
    ...
    if(s1 >= 0 && s1 <= 1 && s2 >= 0 && s2 <= 1 && s3 >= 0 && s3 <= 1 && fequal(sum, 1.0f)){
        ...
        result.tangent = CalculateTangent(P, points[1], points[2]);
        result.bitangent = CalculateBiTan(P, points[1], points[2]);
    }
    return result;
}
Intersection Mesh::RandomSample(float randX, float randY)
{

}

Intersection Triangle::RandomSample(float randX, float randY)
{
...
Intersection Mesh::GetIntersection(Ray r){
    ...
    if(closest.object_hit != NULL)
    {
        ...
        //TODO: Store the tangent and bitangent
        glm::vec4 tri_tan = transform.T()*glm::vec4( tri->CalculateTangent(tri->points[0], tri->points[1], tri->points[2]),0);
        glm::vec4 tri_bitan = transform.T()*glm::vec4( tri->CalculateBiTan(tri->points[0], tri->points[1], tri->points[2]),0);

        closest.tangent = glm::normalize(glm::vec3(tri_tan));
        closest.bitangent = glm::normalize(glm::vec3(tri_bitan));
