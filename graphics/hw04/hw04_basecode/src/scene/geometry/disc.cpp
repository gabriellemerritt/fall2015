...
{
    float a = .5f*transform.getScale().x;
    float b = 0.5f*transform.getScale().y;

    area = a*b*PI;
//    area = 0;
}

Intersection Disc::RandomSample(float randR, float randTheta)
{
    // do some cosine sine stuff
    float sign = 1;
    float randT = 2*PI*randTheta;
    if((randR - 0.5f) < 0)
    {
        sign = -1.0f;
    }
    float random_radius = glm::abs(randR -0.5f);

    float x = sign*glm::sqrt(random_radius )*glm::cos(randT);
    float y = sign*glm::sqrt(random_radius)*glm::sin(randT);
    glm::vec4 isx_point = glm::vec4(x,y,0,1.0f);
...
//    isx.bitangent = NULL; //FIX
//    isx.tangent = NULL; // FIX
...
Intersection Disc::GetIntersection(Ray r)
{
    ...
    if(t > 0 && dist2 <= 0.25f)
    {
        ...
        //TODO: Store the tangent and bitangent  3 random points in unit disk

        glm::vec4 obj_tan = glm::vec4(CalculateTangent(glm::vec3(0,0,0), glm::vec3(-0.4f, 0,0), glm::vec3(0.0f, 0.3f, 0)),0);
        glm::vec4 obj_bit = glm::vec4(CalculateBiTan(glm::vec3(0,0,0), glm::vec3(-0.4f, 0,0), glm::vec3(0.0f, 0.3f, 0)),0);
